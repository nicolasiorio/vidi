...
| 6.1 | Create tunnel + DNS route from Mac | 4 | ✅ Complete | Batch 3 — initial `(.deleted_at|not)` jq filter was wrong (cloudflared uses sentinel `"0001-01-01T00:00:00Z"`, not `null`); replaced with plain `select(.name==$n)` since CLI default already hides deleted tunnels. |
| 6.2 | Install cloudflared on LXC + push creds + config | 3 | ✅ Complete | Batch 3 — Trixie suite not yet published; Bookworm fallback worked. Systemd unit generated by `cloudflared service install` (no committed unit — matches upstream-supported flow). |
| **Phase 7: Theme deploy pipeline** | | | | |
| 7.1 | Write `deploy.sh` (theme override pipeline) | 5 | ✅ Complete | Batch 3 — deviated from plan: hashes the **live remote `default.css`** instead of a `.vidi.hash` marker file. Reason: a marker file goes stale when `make` restores upstream's CSS during re-provision (marker matches new combined hash but actual file is upstream-only); hashing the live file directly catches this. |
| **Phase 8: Idempotency + manual test pass** | | | | |
| 8.1 | Re-run provision; verify no-op | 2 | ✅ Complete | Batch 3 — verified live: every phase no-ops including theme deploy. |
| 8.2 | Cosmetic theme tweak deploy round-trip | 2 | ✅ Complete | Batch 3 — verified live: red-bg deploy → drift detected, restart at `21:50:29`; second deploy → no-op, restart timestamp unchanged. |
